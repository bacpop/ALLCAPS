    ...
    for batch in loader:
        ...
        if capsule_mask.sum() > 0:
            ...
            serotype_indices = torch.tensor(
                [
                    serotype_to_idx[serotype_label[i][-1]]
            ...
            serotype_loss_val = serotype_loss_fn(
                serotype_logits[capsule_mask], serotype_indices
            )
...
def main(args):
    ...
    if args.skip_labels:
        ...
        print(
            f"Skipping labels: {args.skip_labels} accounting for {skip_indices.sum()} samples."
        )
    ...
    if args.hierarchical_loss:
        print(
            "Using hierarchical contrastive loss with weights:",
            weight_fine,
            weight_coarse,
        )
        coarse_labels = (
            labels["Serotype"][indices].apply(map_serotype_to_group).tolist()
        )
    ...
    for fold, (train_idx, test_idx) in enumerate(
        skf.split(np.zeros(len(fine_labels)), fine_labels)
    ):  # Dummy X
        print(f"Fold {fold + 1} / {k_folds}")

        train_ds = ContrastiveChunkedDataset(
            args.embedding_dir,
            np.array(sample_ids)[train_idx],
            np.array(labels_known)[train_idx],
            np.array(is_capsule)[train_idx],
        )
        test_ds = ContrastiveChunkedDataset(
            args.embedding_dir,
            np.array(sample_ids)[test_idx],
            np.array(labels_known)[test_idx],
            np.array(is_capsule)[test_idx],
        )
        ...
        for epoch in tqdm(range(args.epochs), desc=f"Training Fold {fold + 1}"):
            ...
            test_loss, cbl_accuracy, serotype_accuracy = evaluate(
                model,
                test_loader,
                ce_loss_fn,
                serotype_loss_fn,
            ...
            print(
                f"Fold {fold + 1} - Epoch {epoch + 1} - Test Loss: {test_loss:.4f}, CBL Accuracy: {cbl_accuracy:.4f}, Serotype Accuracy: {serotype_accuracy:.4f}"
            )
    ...
    all_ds = ContrastiveChunkedDataset(
        args.embedding_dir, sample_ids, labels_known, is_capsule
    )
    ...
    final_loss, final_cbl_accuracy, final_serotype_accuracy = evaluate(
        model_final,
        all_loader,
        ce_loss_fn,
        serotype_loss_fn,
    ...
    print(
        f"Final model - Loss: {final_loss:.4f}, CBL Accuracy: {final_cbl_accuracy:.4f}, Serotype Accuracy: {final_serotype_accuracy:.4f}"
    )
