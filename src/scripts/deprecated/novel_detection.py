...
def transformer_embedding(  # TODO batch this 
    ...
    for seq in sequences:
        # Chunk the sequence — truncate to max_length to match embed_transformer.py
    ...
    if args.embeddings:
        ...
        for sample in tqdm(range(len(embeddings))):
            ...
            results[sample_id] = {
                ...
                # FIX: use softmax (consistent with training CrossEntropyLoss), not sigmoid
                "is_cbl": torch.softmax(torch.tensor(cbl_logits[sample], dtype=torch.float32), dim=-1)[1].item() > thresholds[0],
            }
        ...
        for record in tqdm(query_sequences, desc="Processing queries"):
            ...
            # FIX: use softmax (consistent with training CrossEntropyLoss), not sigmoid
            cbl_predictions = torch.softmax(torch.tensor(query_cbl_logits, dtype=torch.float32), dim=-1).numpy()
