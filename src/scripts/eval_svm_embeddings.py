...
def parse_args() -> argparse.Namespace:
    ...
    try:
        ...
        if not isinstance(args.model_params, dict):
            logger.warning(
                "model_params must be a JSON object; falling back to defaults."
            )
            args.model_params = {}
...
def load_embeddings_and_labels(
    ...
    if not mask.all():
        missing = (~mask).sum()
        logger.warning("%d entries missing embeddings; they will be dropped.", missing)
...
def evaluate_capsule(
    X: np.ndarray, labels_df: pd.DataFrame, params: Dict
) -> Dict[str, Any]:
...
def evaluate_serotype(
    ...
    logger.info(
        "Dropping %d classes with <2 samples: %s",
        len(unique) - len(valid_classes),
        le.inverse_transform(unique[counts < 2]),
    )
...
def main():
    ...
    with open(args.output, "w") as f:
        f.write("\n".join(reports))
    logger.info("Saved SVM sanity check report to %s", args.output)
