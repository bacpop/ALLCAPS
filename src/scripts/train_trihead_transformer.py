...
def train_one_epoch(model, loader, optimizer, ce_loss_fn, serotype_loss_fn, genogroup_loss_fn, contrastive_loss_fn, alpha, temperature, serotype_to_idx, genogroup_to_idx):
    ...
    for batch in loader:
        ...
        if capsule_mask.sum() > 0:
            serotype_indices = torch.tensor([serotype_to_idx[fine_labels[i]] for i in range(len(capsule_label)) if capsule_mask[i]], device=ce_loss_val.device)
            genogroup_indices = torch.tensor([genogroup_to_idx[coarse_labels[i]] for i in range(len(capsule_label)) if capsule_mask[i]], device=ce_loss_val.device)
...
def main(args):
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(np.zeros(len(fine_labels)), fine_labels)):  # Dummy X
        ...
        genogroup_loss_fn = nn.CrossEntropyLoss()
