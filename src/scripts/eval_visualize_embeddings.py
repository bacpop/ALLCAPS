...
def parse_args():
    ...
    parser.add_argument(
        "--embeddings", type=str, required=True, help="Path to the embeddings."
    )
    parser.add_argument(
        "--labels",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to save the figure (prefix).",
    )
    parser.add_argument(
        "--method",
        type=str,
    ...
    parser.add_argument(
        "--params", type=str, default="{}", help="JSON string of parameters."
    )
    parser.add_argument(
        "--show_noncbl",
        action="store_true",
        default=False,
        help="Include non capsule sequences in the plot.",
    )
    parser.add_argument(
        "--downsample",
        type=int,
        default=None,
        help="Downsample the data for faster plotting.",
    )
    parser.add_argument(
        "--serotypes_list",
        type=str,
        default=None,
        help="Comma-separated list of serotypes to include in the plot.",
    )
    parser.add_argument(
        "--figsize",
        type=str,
    ...
    try:
        ...
        logger.info("Model parameters: %s", args.params)

    try:
        figsize = tuple(map(float, args.figsize.split(",")))
    except ValueError:
        logger.error(
            "figsize must be in 'width,height' format. Using default %s.",
            DEFAULT_FIGSIZE,
        )
...
def main(args):
    logger.info("Starting visualization with method: %s", args.method)
    missing_label, label_column = DEFAULT_MISSING_LABEL, DEFAULT_LABEL_COLUMN
    LABEL_SEP = "|"

    logger.info("Loading the data...")
    embeddings, labels = load_data(
        args.embeddings, args.labels, missing_label=missing_label
    )
    labels = labels.rename({label_column: "Serotype"}, axis=1).fillna(missing_label)
    ...
    if args.serotypes_list:
        ...
        logger.info(
            "Filtering labels to include only: %s, %d rows will be included.",
            serotypes_list,
            serotypes_indices.sum(),
        )
        indices_mask &= serotypes_indices

    if args.downsample:
        logger.info("Downsampling the data for faster plotting...")
        downsample_indices = np.random.choice(
            np.where(indices_mask)[0], size=args.downsample, replace=False
        )
    ...
    if not args.show_noncbl:
        logger.info("Using only capsule locus embeddings (cbl).")
