...
def main(args):
    ...
    logger.info("Running serotype inference...")
    all_predictions = []
    all_probabilities = []
    all_cbl_predictions = []
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(X_filtered), args.batch_size)):
            ...
            # CBL predictions
            cbl_probs = (
                torch.softmax(cbl_logits, dim=1)[:, 1].cpu().numpy()
            )  # Probability of being capsulated
            ...
            if args.collect_energies:
                energies = (
                    (
                        -energy_temperature
                        * torch.logsumexp(serotype_logits / energy_temperature, dim=-1)
                    )
                    .cpu()
                    .numpy()
                )
    ...
    if args.collect_energies and len(all_serotype_energies) > 0:
        ...
        logger.info("Serotype energies saved to: %s", energies_path)
        ...
        if args.save_energy_summary:
            try:
                ...
                percentiles = {
                    p: float(np.percentile(caps_energies, p))
                    for p in [93.0, 95.0, 99.0, 99.5]
                    if len(caps_energies) > 0
                }
                summary = {
                ...
                logger.info("Energy summary saved to: %s", energy_summary_path)
            except Exception as e:
                logger.error("Failed to save energy summary JSON: %s", e)

    predictions_output = f"{args.output_dir}/serotype_predictions.txt"
    labels_df.to_csv(predictions_output, sep="\t", index=False)
    logger.info("Predictions saved to: %s", predictions_output)
    ...
    if args.labels and "Serotype" in labels_df.columns:
        ...
        if not valid_mask.all():
            logger.warning(
                "Removing %d samples with missing true labels", (~valid_mask).sum()
            )
        ...
        if not known_mask.all():
            ...
            logger.warning(
                "Removing %d samples with true labels not in serotype_to_idx: %s",
                n_dropped,
                dropped_labels,
            )
        ...
        if len(y_true) == 0:
            logger.warning("No samples with valid true labels found for evaluation.")
            return

        logger.info("Evaluating on %d samples with valid labels", len(y_true))
        ...
        for serotype in unique_serotypes:
            if serotype in serotype_to_idx:
                ...
                if serotype_mask.any():
                    confidences = labels_df_eval.loc[
                        serotype_mask, f"prob_{serotype}"
                    ].values
        ...
        if not confidence_df.empty:
            ...
            logger.info("Confidence statistics saved to: %s", confidence_output)

        logger.info("Serotype Classification Results:")
        ...
        logger.warning("No evaluation performed (no true labels provided or found).")
        with open(report_path, "w") as f:
            ...
            f.write(
                f"Predicted serotypes: {', '.join(sorted(set(all_predictions)))}\n\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--labels", help="TSV file with true serotype labels indexed by sample ID"
    )
    parser.add_argument(
        "--model", required=True, help="Path to the saved model file (.pth)"
    )
    parser.add_argument(
        "--output_dir", required=True, help="Output directory for classification report"
    )
    parser.add_argument(
        "--device", default="cpu", help="Device to use for inference (cpu or cuda)"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        "--model_params", type=str, default="{}", help="JSON string of model parameters"
    )
    parser.add_argument(
        "--collect_energies",
        action="store_true",
