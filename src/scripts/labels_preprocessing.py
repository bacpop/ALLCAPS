...
from consts import DEFAULT_NONCBL_LABEL
...
def process_labels(raw_path, clean_path):
    ...
    print("Original unique serotypes:")
...
def main():
    ...
    parser.add_argument("--raw-labels", required=True, help="Path to raw labels TSV file")
    ...
    result = process_labels(args.raw_labels, args.output_dir / "cleaned_labels.tsv")
    ...
    # Add capsule presence column and contig id column
    result['Is_capsule'] = 1
    noncbl_names = [f.split(".")[0] for f in os.listdir(output_path / 'base_embeddings_chunked/non-cbl')]
    public_ids, contig_ids = zip(*[name.split(CONTIG_SEP) for name in noncbl_names])
    noncbl_df = pd.DataFrame(
        {
            'Public_ID': public_ids,
            'Contig_ID': contig_ids,
            'Serotype': [non_cbl_label] * len(public_ids),
            'Is_capsule': [0] * len(public_ids)
        })  # concatenate non-CBL entries
    result = pd.concat([result, noncbl_df], ignore_index=True)
    result['Contig_ID'] = result['Serotype'].apply(lambda x: f"contig_{x}" if pd.notna(x) else np.nan)
