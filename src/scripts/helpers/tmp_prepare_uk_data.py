...
def parse_args():
    parser = argparse.ArgumentParser(description="Prepare UK dataset metadata and FASTA files")
    parser.add_argument("--metadata", required=True, help="Path to the UK dataset metadata CSV file")
    parser.add_argument("--fasta", required=True, help="Path to the UK dataset FASTA file")
    parser.add_argument("--output_dir", default=".", help="Directory to save processed files")
    parser.add_argument("--output_metadata", default="uk_metadata.csv", help="Path to save the processed metadata CSV file")
    parser.add_argument("--output_fasta", default="uk_sequences.fasta", help="Path to save the processed FASTA file")
...
def main():
    ...
    contig_ids = list(map(int, [rid[rid.find("contig") + 6:rid.find("contig") + 9] for rid in record_ids]))
    ...
    new_records = []
    for record, sample_name, contig_id, new_record_description in zip(fasta_records, sample_names, contig_ids, new_record_desc):
        record.id = f"UK|{sample_name}{CONTIG_SEP}{contig_id}"
        record.description = new_record_description
        new_records.append(record)

    metadata = pd.read_csv(args.metadata)[["ENA", "Agglutination"]] \
        .rename(columns={"ENA": "Public_ID", "Agglutination": "Serotype"})
